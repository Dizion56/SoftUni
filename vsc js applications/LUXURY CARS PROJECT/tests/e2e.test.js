  ...
  catalog: [
    {
      ...
      speed: "360",
      about:
        "The Pagani Huayra is a breathtaking hypercar that seamlessly blends art and engineering, representing the epitome of automotive craftsmanship. Its striking exterior features aerodynamic curves and gull-wing doors, showcasing a design that's both futuristic and elegant. Underneath the hood, a meticulously crafted AMG-sourced V12 engine delivers an exhilarating performance, propelling the Huayra to astounding speeds, while the opulent interior envelops the driver in a cocoon of luxury, making every journey a symphony of power and refinement.",
    },
    {
      _id: "1002",
      _ownerId: "0002",
...
const endpoints = {
  ...
  own: (motorcycleId, userId) =>
    `/data/cars?where=_id%3D%22${motorcycleId}%22%20and%20_ownerId%3D%22${userId}%22&count`,
};
...
describe("E2E tests", function () {
  ...
  describe("Navigation bar [ 10 Points ]", () => {
    it("Logged user should see correct navigation [ 2.5 Points ]", async function () {
      ...
      //Test for navigation
      await page.waitForSelector("nav >> text= Our Cars", {
        timeout: interval,
      });
    ...
    it("Guest user should see correct navigation [ 2.5 Points ]", async function () {
      ...
      expect(await page.isVisible("nav"), "Dashboard is not visible").to.be
        .true;
      expect(
        await page.isVisible("nav >> text=Add Your Car"),
        "Create is visible"
      ).to.be.false;
    ...
    it("Guest user navigation should work [ 2.5 Points ]", async function () {
      ...
      let funFactsBtn = await page.waitForSelector("nav >> text=Our Cars", {
        timeout: interval,
      });
    ...
    it("Logged in user navigation should work [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector("#dashboard", { timeout: interval });
      let createBtn = await page.waitForSelector("nav >> text=Add Your Car", {
        timeout: interval,
      });
  ...
  describe("Home Page [ 5 Points ]", function () {
    it("Show Home page text [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector(
        "text=Accelerate Your Passion Unleash The Thrill Of Sport Cars Together!",
        { timeout: interval }
      );
      expect(
  ...
  describe("Dashboard Page [ 15 Points ]", function () {
    ...
    it("Check Our Cars page with 0 Motorcycles [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector("text=Nothing to see yet", {
        timeout: interval,
      });
    ...
    it("Check Cars have correct images [ 2.5 Points ]", async function () {
      ...
      const images = await page.$$eval(".car img", (t) => t.map((s) => s.src));
  ...
  describe("CRUD [ 50 Points ]", () => {
    describe("Create [ 12.5 Points ]", function () {
      it("Create does NOT work with empty fields [ 2.5 Points ]", async function () {
        ...
        let addFactButton = await page.waitForSelector("text=Add Your Car", {
          timeout: interval,
        });
      ...
      it("Create makes correct API call [ 2.5 Points ]", async function () {
        ...
        let speedElement = await page.waitForSelector('[name="speed"]', {
          timeout: interval,
        });
    ...
    describe("Details [ 10 Points ]", function () {
      it("Details calls the correct API [ 2.5 Points ]", async function () {
        ...
        let moreInfoButton = await page.waitForSelector(
          `.car:has-text("${data.model}") >> .details-btn`,
          { timeout: interval }
        );
      ...
      it("Details with guest calls shows correct info [ 2.5 Points ]", async function () {
        ...
        let priceElement = await page.waitForSelector(".price", {
          timeout: interval,
        });
        let weightElement = await page.waitForSelector(".weight", {
          timeout: interval,
        });
        let speedElement = await page.waitForSelector(".top-speed", {
          timeout: interval,
        });
        let aboutElement = await page.waitForSelector("#car-description", {
          timeout: interval,
        });
    ...
    describe("Edit [ 17.5 Points ]", function () {
      ...
      it("Edit does NOT work with empty fields [ 2.5 Points ]", async function () {
        ...
        let mileageInput = await page.waitForSelector('[name="weight"]', {
          timeout: interval,
        });
        let contactInput = await page.waitForSelector('[name="speed"]', {
          timeout: interval,
        });
